...
from Ner.CTIMrc.datat_Pretreatment import pretreatment
from Ner.CTIMrc.loss import DiceLoss
from Ner.model.CTImrc import Bert_Mrc
from Ner.CTIMrc import config
from Ner.CTIMrc.optimizer import optimizer
import numpy as np
...
class Train():
    def __init__(self):
        ...
        self.trainloader = pre.loader(input_ids[:config.train_size], starts[:config.train_size], ends[:config.train_size],
                                      type_ids[:config.train_size], attention_masks[:config.train_size],
                                      match_labels[:config.train_size],qs[:config.train_size],kg_matrixs[:config.train_size])
    ...
    def train(self):
        ...
        for data in self.trainloader:
            tokens_tensors, starts_tensors,ends_tensors,type_ids_tensors, masks_tensors,match_labels,type_,kg_matrixs = [t for t in data]
            self.optimizer.zero_grad()
            start_logits, end_logits, span_logits = self.model(input_ids=tokens_tensors.cuda(),token_type_ids=type_ids_tensors.cuda(),attention_mask=masks_tensors.cuda(),kg_matrixs=kg_matrixs.cuda())
    ...
    def dev(self):
        ...
        for batch in self.devloader:
            tokens_tensors, starts_tensors,ends_tensors,type_ids_tensors, masks_tensors,match_labels,type_ ,kg_matrixs= [t for t in batch]
            with torch.no_grad():
                start_logits, end_logits, span_logits = self.model(input_ids=tokens_tensors.cuda(),token_type_ids=type_ids_tensors.cuda(),attention_mask=masks_tensors.cuda(),kg_matrixs=kg_matrixs.cuda())
    ...
    def Print_result(self,label,pred,valid):
        f1_ = f1_score(pred,valid,average='macro')
