...
stop_word = ['APT C', 'APT', 'Android', 'macOS']
...
def cve_test(text):
    res = re.findall('CVE\-[0-9]{4}\-[0-9]{4,6}',text)
    return res

def gen_cloud(dicts, key):
    ...
    wc.generate_from_frequencies(dicts)  # 加载词云文本
    wc.to_file(f"{key}_WordCloud.png")  # 保存词云文件


if __name__ == '__main__':
    texts = []
    dicts = {'val':[], 'malware':[], 'attacker':[]}
    for i in range(1, config.txt_len + 1):
        with open(f'../../Ner_data/Ner_data1757/{i}.json', 'r', encoding='utf-8') as f:
            file = json.load(f)
        content = file['content']
        if len(content) < config.max_len:
            texts.append(content)
    print(len(texts))
    for text in texts:
        # new_dicts = ner_test([text])
        # for key in ['attacker', 'malware']:
        #     dicts[key] += new_dicts[key]
        new_dicts2 = cve_test(text)
        dicts['val'] += new_dicts2

    from collections import Counter
    for key in dicts:
        gen_cloud(Counter(dicts[key]), key)
