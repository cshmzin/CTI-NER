...
class Bert_Mrc(BertPreTrainedModel):
    ...
    def Kg_Attention(self,kg_matrixs,sequence_heatmap):
        Q = kg_matrixs.unsqueeze(-1) #[batch,seq_len,1]
        Q = Q.squeeze(2)
        Q = F.softmax(Q,dim=1)
        Q = Q.unsqueeze(2)

        K = sequence_heatmap.transpose(1,2) #[batch,hidden,seq_len]
        V = sequence_heatmap #[batch,seq_len,hidden]
        a = K.bmm(Q) #[batch,hidden,1]
        a = a.squeeze(2).unsqueeze(1) #[batch,1,hidden]
        out = V*a #[batch,seq_len,hidden]
        return out


    def forward(self, input_ids, token_type_ids=None, attention_mask=None,kg_matrixs = None):

        bert_outputs = self.bert(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)

        sequence_heatmap = bert_outputs[0]  # [batch, seq_len, hidden]
        #kg = kg_matrixs.unsqueeze(-1) #[batch,seq_len,1]
        #sequence_heatmap = torch.cat((sequence_heatmap,kg),dim=-1) #[batch,seq_len,hidden+1]
        #sequence_heatmap = sequence_heatmap*kg
        sequence_heatmap = self.Kg_Attention(kg_matrixs,sequence_heatmap)
