import torch
from seqeval.metrics import f1_score as f1, precision_score, recall_score, accuracy_score
import os

from Ner.RoBerta_wwm_crf.datat_Pretreatment import pretreatment, Dataset
from Ner.model.Roberta_wwm import BertLstmCrf

from Ner.RoBerta_wwm_crf import config
from Ner.RoBerta_wwm_crf.optimizer import optimizer
...
class Train():
    def __init__(self):
        ...
        self.model = BertLstmCrf.from_pretrained("hfl/chinese-roberta-wwm-ext", num_labels=config.num_labels)
        self.model.cuda()
    ...
    def run(self):
        if os.path.exists('RoBerta_wwm_crf.pth'):
            self.model.load_state_dict(torch.load('RoBerta_wwm_crf.pth'))
            print('use load_state')
        for epoch in range(self.Epochs):
            self.train()
            self.dev()
        torch.save(self.model.state_dict(), 'RoBerta_wwm_crf.pth')
        print(self.losses)
        print(self.f1es)
    ...
    def dev_result(self):
        if os.path.exists('RoBerta_wwm_crf.pth'): self.model.load_state_dict(torch.load('RoBerta_wwm_crf.pth'))
        self.model.eval()
        predictions, true_labels = [], []
